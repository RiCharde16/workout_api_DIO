...
async def query(db_session: DatabaseDependency, nome: str | None = None, cpf: str | None = None) -> list[AtletaOut]:
    ...
    print(nome)
    if cpf:
        atletas = (await db_session.execute(select(AtletaModel).filter_by(cpf=cpf))).scalars().all()

    elif nome:
        atletas = (await db_session.execute(select(AtletaModel).filter(AtletaModel.nome.like(f'%{nome}%')))).scalars().all()
